...
class CRAIGStrategy(DataSelectionStrategy):
    ...
    def compute_score(self, model_params, idxs):
        ...
        subset_loader = torch.utils.data.DataLoader(trainset, batch_size=self.trainloader.batch_size, shuffle=False,
                                                    ...
                                                    pin_memory=True, collate_fn=self.trainloader.collate_fn, num_workers=8)
    ...
    def select(self, budget, model_params):
        ...
        end_time = time.time()
        self.logger.info("CRAIG strategy data selection time is: %.4f", end_time-start_time)
        # print(total_greedy_list)
